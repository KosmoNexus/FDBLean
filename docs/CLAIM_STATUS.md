# FDBLean Claim Status. Update 1 AUG 2026
...
- Named theorem and lemma declarations: **293**
- Formal modules represented in the theorem index: **22**
- `sorry`: **0**
- `native_decide`: **0**
- Capstone theorem object: `FDBLean.mainTheoremChain`
- Negative-result module: `FDBLean.FanoObstruction`
...
### Fano edit-reachability obstruction

- The ambient three-bit profile space has 7 projective points and 7 projective lines.
- Directed editing reaches exactly the 8 coordinate subspaces of `F₂³`.
- Exactly 3 of the 7 projective points are edit reachable.
- Exactly 3 of the 7 projective lines are edit reachable.
- Four projective lines are missed.
- `GL(3,F₂)` has order 168.
- Exactly 6 invertible linear maps preserve the edit-reachable subspace family.
- The remaining 162 collineations do not preserve biological edit reachability.
- Therefore the recoding dynamics does not generate the full Fano-plane incidence structure.
- The obstruction is the preferred coordinate basis selected by codon position.
...
## 16. Fano edit-reachability obstruction

### `FanoObstruction.lean`
- Concrete kernel-reducible model of `F₂³`
- Seven projective points and seven projective lines
- Exact characterization of the 8 coordinate subspaces reached by editing
- Three reachable projective points
- Three reachable projective lines
- Four missed projective lines
- Verification that `GL(3,F₂)` has order 168
- Six collineations preserving the edit-reachable family
- 162 collineations failing to preserve biological reachability
- Formal rejection of a full Fano-plane correspondence generated by recoding

## 17. Capstone theorem object

### `MainTheoremChain.lean`
- One typed object collecting the principal theorem chain
- Includes the Fano edit-reachability obstruction
...
### `EditingTrichotomy.lean` — 12 theorems
...
### `OriginNarrowing.lean` — 16 theorems
...
### `TaggedNucleotide.lean` — 19 theorems
...
### `PairClassification.lean` — 21 theorems
...
### `PositionalChargeLaw.lean` — 12 theorems
...
## 21. Fano Edit-Reachability Obstruction

### `FanoObstruction.lean` — 21 theorems

272. `Vec.get_zero` — L68  
273. `Vec.get_one` — L74  
274. `Vec.get_two` — L80  
275. `points_card` — L137  
276. `lines_card` — L142  
277. `editableBool_eq_true_iff` — L164  
278. `coordinateSubspace_isSubspace` — L184  
279. `reachable_isSubspace` — L207  
280. `editableSupport_codonForSupport` — L225  
281. `reached_eq_coordinateSubspaces` — L259  
282. `reached_card` — L280  
283. `reachedPoints_card` — L300  
284. `reachedLines_card` — L308  
285. `coordinateSubspace_isCoordinate` — L346  
286. `reached_all_coordinate` — L354  
287. `lines_missed` — L366  
288. `glMatrices_card` — L454  
289. `permutationMatrices_card` — L483  
290. `preservingGL_card` — L500  
291. `unrealized_collineations` — L510  
292. `fano_obstruction_summary` — L517  

---

## 22. Capstone Theorem Chain

### `MainTheoremChain.lean` — 1 theorem

293. `mainTheoremChain` — L204  
...
| Complement transport, RNA/DNA transport, and tagging | 56 |
| Pair classification and error control | 35 |
| Amino acids, code, and charge | 34 |
| Fano edit-reachability obstruction | 21 |
| Capstone | 1 |
| **Total** | **293** |
